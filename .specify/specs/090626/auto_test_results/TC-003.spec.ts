...
// TC-003 — Cấu hình IsConsecutive = true thành công
// PBI: 090626 | Group: Cấu hình | Priority: P1

test('TC-003 — Cấu hình IsConsecutive = true thành công', { tag: ['@090626', '@auto'] }, async ({ page }) => {
  // PRECONDITION: Loại ngày nghỉ datafake.consecutive.leave_type_name ("Nghỉ hiếu TEST") có IsConsecutive = false
  ...
  // Bước 2: Bật checkbox IsConsecutive = true
  await catPage.inputCheckbox(page.locator('input[name="IsConsecutive"]'), true, 'Đăng ký liên tiếp')
  ...
  await row2.click()
  await expect(page.locator('input[name="IsConsecutive"]')).toBeChecked()
})
