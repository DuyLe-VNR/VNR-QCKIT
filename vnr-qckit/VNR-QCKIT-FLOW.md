...
**Luồng xử lý (3 bước):**
```
Bước 1  → Gọi qc_setup (tuần tự — Playwright config + Chromium + global-setup + BasePage URL check)
...
| File | Bắt buộc | Ghi chú |
|---|---|---|
| `.specify/memory/components/component_{alias}.md` | ✅ | **BẮT BUỘC** — thiếu → DỪNG |
| `.specify/tests/url-aliases.md` | ✅ | **BẮT BUỘC** — alias → path + group detection |
| `.specify/rules/component-rule.md` | ✅ | **BẮT BUỘC** — Selector rules |
...
| `.specify/tests/playwright/{PBI_ID}/datafake.json` | ✅ | Đọc trước khi sinh spec.ts |

**OUTPUT:**
| File | Trạng thái | Ghi chú |
|---|---|---|
| `.specify/specs/{PBI_ID}/testcase.md` | Cập nhật | Cột Trạng thái + cột ⭐ trong Mục 4 |
| `.specify/tests/playwright/{PBI_ID}/{PBI_ID}.spec.ts` | Tạo mới | Spec batch — tất cả TC trong 1 file |
| `.specify/tests/playwright/{PBI_ID}/{TC_ID}.spec.ts` | Tạo mới | Spec đơn lẻ — 1 file / 1 TC (rerun) |
...
| `.specify/specs/{PBI_ID}/auto_test_results/run-{DATETIME}-summary.md` | Tạo mới | Tóm tắt lần chạy (bao gồm Runtime Context, Data Chaining, Exploratory Warnings) |
...
| **Auth guard check** | `playwright-cli evaluate "window.location.href"` |
| **Capture URL/ID/text từ DOM** | `playwright-cli evaluate "..."` |
...
| `qc_map_flow` | `component_{alias}.md`, `component-rule.md`, `userflow-{group}.md` | `{AliasName}Locator.ts`, `{AliasName}Page.ts`, `locator-map.md` | Ghi đè |
...
| `qc_generate` | `spec.md`, `plan.md` (**cả hai bắt buộc**), `userflow-{group}.md` | `testcase.md` (Mục 4), `playwright/{PBI_ID}/datafake.json`, `knowledge/*.md`, `_IMPACT_INDEX.json` | Tạo mới |
...
│   ├── playwright/                  ← testDir cho spec files
│   │   └── {PBI_ID}/
